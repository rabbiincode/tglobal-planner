...
export const CalendarGrid = () => {
  ...
  const getEventRowSpan = (startTime: string, endTime: string) => {
    ...
    const span = Math.max(resolvedEndIndex - (startIndex === -1 ? 0 : startIndex), 1);

    return { startRow: (startIndex === -1 ? 1 : startIndex + 2), rowSpan: span };
  };

  const eventsByRoom = filteredEvents.reduce<Record<string, typeof filteredEvents>>((acc, event) => {
  ...
  PLANNER_ROOMS.forEach((room, roomIndex) => {
    ...
    const spans: number[] = [];
    for (let i = 0; i < numEvents; i++) spans.push(baseSpan + (i < remainder ? 1 : 0));
  ...
  return (
    <Flex w="full" h="full" overflow="auto" bg="white" rounded="xl" borderWidth="1px" borderColor="brandNeutralOutline" maxH="calc(100vh - 220px)">
        ...
        <ScrollArea.Viewport>
          <ScrollArea.Content display="flex" flexDirection="row">
            <VStack border="none" gap={0} justifyContent="start">
              ...
              <Grid templateColumns="120px" templateRows={`repeat(${PLANNER_TIME_SLOTS.length}, ${ROW_HEIGHT}px)`}>
                {PLANNER_TIME_SLOTS.map(slot => (
                  <Box key={slot.time} borderRightWidth="1px" borderBottomWidth="1px" borderColor="brandNeutralOutline" p={2} px={4} color="brandNeutralGrey" fontSize="xs" fontWeight="semibold" position="sticky" left={0} bg="white" zIndex={10} display="flex" alignItems="flex-start" pt={2}>
                    {slot.time}
                  </Box>
                ))}
              </Grid>
            ...
            >
              ...
              {filteredEvents.map(event => {
                ...
                return (
                  <Box key={event.id} gridColumn={eventGridColumns[event.id]} gridRow={`${startRow} / span ${rowSpan}`} zIndex={5}>
                    <EventCard
                      title={event.title}
